...
export class AddressController {
  ...
  async fetchAddress(req: AuthRequest, res: Response) {
    try {
      const { success, data, message } = await this.addressService.fetchaddress(
        req.user as string
      );
  ...
  async createAddress(req: AuthRequest, res: Response) {
    try {
      const { message, success } = await this.addressService.createAddress(
        req.user as string,
        req.body
      );
  ...
  async deleteAddress(req: AuthRequest, res: Response) {
    try {
      const { message, success } = await this.addressService.deleteAddress(
        req.params.id
      );
      if (success) res.status(HttpStatus.OK).json({ message, success });
      else res.status(HttpStatus.BAD_REQUEST).json({ message, success });
